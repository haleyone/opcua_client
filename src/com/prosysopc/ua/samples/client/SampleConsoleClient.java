...
//		sampleConsoleClient.initialize(args);
	...
	public static void usage() {
		...
		println("   opc.tcp://localhost:52530/OPCUA/SampleConsoleServer (Prosys Sample Server)");
	...
	protected String defaultServerUri = "opc.tcp://localhost:53530/OPCUA/SampleConsoleServer";
	...
	public String serverUri = "opc.tcp://localhost:53530/OPCUA/SampleConsoleServer";
	 ...
	 */
    public NodeId browse(NodeId nodeId, NodeId prevId) throws ServiceException, StatusException {
	 ...
	 * @throws StatusException
	 */
protected String createMonitoredItem(Subscription sub, NodeId nodeId, UnsignedInteger attributeId)
...
//                        System.out.println("requestedSamplingInterval: "+requestedSamplingInterval);
//                        UnsignedInteger s = sub.getLastSequenceNumber();
//                        System.out.println("getLastAlive:"+s);
			sub.addItem(item);
			monitoredItemId = item.getMonitoredItemId();
			double revisitedSamplingInterval = item.getSamplingInterval();
                        System.out.println("revisitedSamplingInterval: "+revisitedSamplingInterval);
			if (Double.isInfinite(revisitedSamplingInterval))//正无穷大
				println("Error, got infinite number as revised sampling interval");
			else if (Double.isNaN(revisitedSamplingInterval))//负无穷大
        ...
        return sub.getSubscriptionId()+" "+monitoredItemId;
	}
	 ...
	 * @throws StatusException
	 */
	protected Subscription createSubscription2(int publishTime) throws ServiceException, StatusException {
		// Create the subscription
		Subscription subscription = new Subscription();

		// Default PublishingInterval is 1000 ms

		subscription.setPublishingInterval(publishTime);
//                subscription.setMaxKeepAliveCount(ACTION_ALL);
...
//        System.out.println("publish:"+publishTime);
...
//				analogInfoStr);
	...
	public void initialize() throws URISyntaxException, SecureIdentityException, IOException,
//	public void initialize(String[] args) throws URISyntaxException, SecureIdentityException, IOException,
		...
		do {
			...
			try {
				...
				case 7:
					...
					break;
				case 8:
//					subscribe(nodeId,publishTime,attributeNum);
					break;
				case 9:
	 ...
	 * @return
	 */
	public boolean parseCmdLineArgs(String[] args) throws IllegalArgumentException {
		int i = 0;
		...
		if (client.isConnected())
			...
			try {
				...
				if (currentNodeStr != null) {
					println("-----------"+currentNodeStr+"---------"+currentNodeStr.length());
					println("");
				}
			} catch (ServiceException e) {
				printException(e);
	...
	public String getValue(DataValue value,UnsignedInteger attributeId){
		StringBuilder sb = new StringBuilder();
		if (value.getStatusCode().isNotBad()) {
//			sb.append(" | Value: ");
			if (value.isNull())
...
//									sb.append("(" + dataType.getDisplayName().getText() + ")");
	...
	public List read(NodeId nodeId) throws ServiceException, StatusException {
		List<Read> listRead = new ArrayList<Read>();
		for (long i = Attributes.NodeId.getValue(); i <= 5; i++){
			UnsignedInteger attributeId = UnsignedInteger.valueOf(i);
			
			DataValue value = client.readAttribute(nodeId, attributeId);
			
			String attribute = AttributesUtil.toString(attributeId);
//			System.out.println(attribute+"-----------");
			StatusCode statusCode = value.getStatusCode();
			String v = getValue(value,attributeId);
			String time = dateTimeToString("",value.getServerTimestamp(), value.getServerPicoseconds());
			String statusName = statusCode.getName();
			
			r = new Read(nodeId,attribute,statusName,v,time);
			listRead.add(r);
		}
//		System.out.println(r.getReadList().size()+"---------------");
	 ...
	 * @return
	 */
        protected UnsignedInteger readAttributeId2(int attributeNum) {
            UnsignedInteger attributeId = UnsignedInteger.valueOf(attributeNum);
            return attributeId;
        }
	protected UnsignedInteger readAttributeId() {
		for (long i = Attributes.NodeId.getValue(); i <= 5; i++){
			System.out.println(i+" "+ AttributesUtil.toString(UnsignedInteger.valueOf(i))+"------");
		}
	...
	/*	int action = readAction();
		if (action < 0)
			return null;*/
		UnsignedInteger attributeId = UnsignedInteger.valueOf(2);
...
//				referenceTypeStr, r.getBrowseName(), r.getIsForward() ? "" : " [Inverse]");
		return r.getDisplayName().getText()+"---"+typeStr;
	}
	 ...
	 */
	String subItemId;
	public String subscribe(NodeId nodeId,int publishTime,int attributeNum) {
		...
		println("");
		UnsignedInteger attributeId = readAttributeId2(attributeNum);
//        System.out.println("attributeNum: "+attributeNum);
		if (attributeId != null) {
			try {
				// Create the subscription
				if (subscription == null){
					subscription = createSubscription2(publishTime);
				}
				subItemId = createMonitoredItem(subscription, nodeId, attributeId);
//				System.out.println(subItemId+"..........................");
	...
	//SetTree
//	Opc_Ua_Client opc_client;
	public void createTreeNode(NodeId nodeId){
		String currentNode = getCurrentNode(nodeId);
//		System.out.print(currentNode+"============");
//		DefaultMutableTreeNode Root=(DefaultMutableTreeNode)(Object)currentNode;
//		Root = new DefaultMutableTreeNode(currentNode);
		List<ReferenceDescription> references;
		// Find the reference to use for browsing up: prefer the previous node,
		// but otherwise accept any hierarchical inverse reference
//		List<ReferenceDescription> upReferences;
		...
		try {
			client.getAddressSpace().setMaxReferencesPerNode(1000);
			references = client.getAddressSpace().browse(nodeId);
			System.out.println(references.size()+"个Child");
			
			if(references.size() > 0){
				for (int i = 0; i < references.size(); i++){
					r = references.get(i);
					NodeId nodeid = client.getAddressSpace().getNamespaceTable().toNodeId(r.getNodeId());
					String child = nodeid.toString();
//					Root.add(new DefaultMutableTreeNode(child));
//					opc_client.SetTree2(currentNode,child);
//					createTreeNode(client.getAddressSpace().getNamespaceTable().toNodeId(r.getNodeId()));					
//					return nodeid;
				}
			}else{
				
//				opc_client.SetTree2(currentNode, null);
			}
//			createTreeNode(client.getAddressSpace().getNamespaceTable().toNodeId(r.getNodeId()));
...
//		String []childname = new String[child.length];
//		DefaultMutableTreeNode Root=(DefaultMutableTreeNode)(Object)root;
//		for(int i=0; i<child.length; i++){
//			Root.add(new DefaultMutableTreeNode(child[i]));
//		}
	}
	public String getCurrentNode(NodeId nodeId){
		if (client.isConnected())
			// Find the node from the NodeCache
			try {
				UaNode node = client.getAddressSpace().getNode(nodeId);
